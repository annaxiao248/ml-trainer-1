...
/**
 * Web Bluetooth implementation for Android/Desktop devices
 */
export class WebMicrobitBluetooth implements MicrobitConnection {
  ...
  async connect(...states: DeviceRequestStates[]): Promise<void> {
    logEvent({
      ...
      action: 'Bluetooth connect start (Web)',
      states,
    });
    ...
    try {
      ...
      logEvent({
        type: this.isReconnect ? 'Reconnect' : 'Connect',
        action: 'Bluetooth connect success (Web)',
        states,
      });
    } catch (e) {
      ...
      logEvent({
        type: this.isReconnect ? 'Reconnect' : 'Connect',
        action: 'Bluetooth connect failed (Web)',
        states,
      });
